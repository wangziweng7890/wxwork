...
function requestHook(config, isDwp) {
  ...
  const dwpToken =
    userStore.getDwpToken ||
    JSON.parse(localStorage.getItem('user') || '{}')?.dwpToken
  config.headers['Token'] =
    config.headers['Token'] || (isDwp ? dwpToken : userStore.getToken)
  // post，put请求前处理
  if (
...
function responseHook(res) {
  ...
  // 其他状态码
  !res.config.hideError && showToast({ message: data.message || data.msg, icon: 'cross' })
...
function createHttp(baseURL, isDwp) {
  ...
  // 返回响应数据拦截
  http.interceptors.response.use(responseHook, responseError)
  return http
}
